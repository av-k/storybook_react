...
export const FormCreateOptions = {
	mapPropsToFields: {
		...
		description: 'Convert props to field value(e.g. reading the values from Redux store). And you must mark returned fields with Form.createFormField. Please note that the form fields will become controlled components. Properties like errors will not be automatically mapped and need to be manually passed in.'
	},
	name: {
		type: 'string',
	...
	onFieldsChange: {
		...
		description: 'Specify a function that will be called when the fields (including errors) of a Form.Item gets changed. Usage example: saving the field\'s value to Redux store.'
	},
	onValuesChange: {
	...
	getFieldsError: {
		...
		description: 'Get the specified fields\' error. If you don\'t specify a parameter, you will get all fields error.'
	},
	getFieldsValue: {
		type: 'Function([fieldNames: string[]])',
		description: 'Get the specified fields values. If you don\'t specify a parameter, you will get all fields values.'
	},
	getFieldValue: {
	...
	isFieldsTouched: {
		...
		description: 'Check whether any of fields is touched by getFieldDecorator\'s options.trigger event'
	},
	isFieldTouched: {
		type: '(name: string) => boolean',
		description: 'Check whether a field is touched by getFieldDecorator\'s options.trigger event'
	},
	isFieldValidating: {
		type: 'Function(name)',
	...
	resetFields: {
		type: 'Function([names: string[]])',
		description: 'Reset the specified fields value(to initialValue) and status. If you don\'t specify a parameter, all the fields will be reset.'
	},
	setFields: {
	...
	setFieldsValue: {
		...
		description: 'Set the value of a field. (Note: please don\'t use it in componentWillReceiveProps, otherwise, it will cause an endless loop, reason)'
	},
	validateFields: {
		type: `(
	[fieldNames: string[]],
	...
	callback(errors, values)
		...
		description: 'Validate the specified fields and get theirs values and errors. If you don\'t specify the parameter of fieldNames, you will validate all fields.'
	},
	validateFieldsAndScroll: {
		type: 'same as validateFields',
		description: 'This function is similar to validateFields, but after validation, if the target field is not in visible area of form, form will be automatically scrolled to the target field area.'
	}
}
...
export const validateFields = {
	'options.first': {
		type: 'boolean',
		description: 'validateFields/validateFieldsAndScroll option. If true, every field will stop validation at first failed rule'
	},
	'options.firstFields': {
		type: 'String[]',
		description: 'validateFields/validateFieldsAndScroll option. Those fields will stop validation at first failed rule'
	},
	'options.force': {
		type: 'boolean',
		description: 'validateFields/validateFieldsAndScroll option. Should validate validated field again when validateTrigger is been triggered again'
	},
	'options.scroll': {
		type: 'Object',
		description: 'validateFields/validateFieldsAndScroll option. Config scroll behavior of validateFieldsAndScroll, more: dom-scroll-into-view\'s config'
	},
}
...
export const getFieldDecorator = {
	id: {
		type: 'string',
		description: 'getFieldDecorator(id, options). The unique identifier is required. support nested fields format.'
	},
	'options.getValueFromEvent': {
		type: 'function(...args)',
		description: 'getFieldDecorator(id, options). Specify how to get value from event or other onChange arguments'
	},
	'options.getValueProps': {
		type: 'function(value): any',
		description: 'getFieldDecorator(id, options). Get the component props according to field value.'
	},
	'options.initialValue': {
		type: '',
		description: 'getFieldDecorator(id, options). You can specify initial value, type, optional value of children node. (Note: Because Form will test equality with === internally, we recommend to use variable as initialValue, instead of literal)'
	},
	'options.normalize': {
		type: 'function(value, prevValue, allValues): any',
		description: 'getFieldDecorator(id, options). Normalize value to form component, a select-all example'
	},
	'options.preserve': {
		type: 'boolean',
		description: 'getFieldDecorator(id, options). Keep the field even if field removed'
	},
	'options.rules': {
		type: 'object[]',
		description: 'getFieldDecorator(id, options). Includes validation rules. Please refer to "Validation Rules" part for details.'
	},
	'options.trigger': {
		type: 'string',
		description: 'getFieldDecorator(id, options). When to collect the value of children node'
	},
	'options.validateFirst': {
		type: 'boolean',
		description: 'getFieldDecorator(id, options). Whether stop validate on first rule of error for this field.'
	},
	'options.validateTrigger': {
		type: 'string|string[]',
		description: 'getFieldDecorator(id, options). When to validate the value of children node.'
	},
	'options.valuePropName': {
		type: 'string',
		description: 'getFieldDecorator(id, options). Props of children node, for example, the prop of Switch is \'checked\'.'
	}
}
